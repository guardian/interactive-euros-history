...
export default function sankeyChart(d3, options, margin, rData) {
        ...
        dataArr = getDataArr(options.year, rData);
				...
				var container=d3.select(options.container)
					.append("div")
					.attr("id", options.chartContainer)
					.attr("class", "chart-container")
					.html(chartHTML);    

				 d3.select("#"+options.chartContainer+" .info").html("Euro "+options.year);	 

        var formatNumber = d3.format(",.0f"),
            format = function(d) { return formatNumber(d) + " TWh"; },
            color = d3.scale.category20();

       var svg = d3.select("#"+options.chartContainer+" .matches").append("svg")
...
// include d3 sankey in a seperate file later
  ...
  function computeNodeLinks() {
    ...
    links.forEach(function(link){
      ...
      //only pushing none-null values - CF
      if(link.target){ 
